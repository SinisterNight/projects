//Dewey Holland
//HW2 pt2
// This program follows the directions of question number 3 to the best of my ability comments are included on just about every step to describe what is happening through the program. This section covers after a mutex lock is added so that the threads can run concurrently while not starting a race condition. that being said a lock is being used in this section this is part 2 of number 3
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
//imports
...
FILE* file;
pthread_mutex_t mutex; //named mutex for simplicity <--------------------------------------------------------
...
void* process(void* arg) { //method creation for process
    ...
    for (int i = 0; i < 200; i++) {//iterate 0 200
        pthread_mutex_lock(&mutex);//locking the mutex where it starts the race condition <------------------
...
int main() {//main
    ...
    pthread_mutex_destroy(&mutex);//destroy the mutex this is good practice <---------------------------------
    return 0;//return 0
}//end main
